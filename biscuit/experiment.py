...
def train(project, hp, label, filters, save_predictions=False,
    ...
    """
    project.train(
        (utils.OUTCOME if 'outcome' not in kwargs else kwargs['outcome']),
        exp_label=label,
        filters=filters,
        params=hp,
...
def train_nested_cv(project, hp, label, outer_k=3, inner_k=5, **kwargs):
    ...
    Returns:
        None
    """
    k_models = utils.find_cv(project, label, k=outer_k)
...
def plot_uncertainty_calibration(project, exp, tile_thresh, slide_thresh, pred_thresh):
    ...
    val_dfs = [
        pd.read_csv(
            join(
                utils.find_model(project, f'EXP_{exp}_UQ', kfold=k),
    ...
    for v in range(len(val_dfs)):
        rename_cols = {
            utils.y_pred_header: 'y_pred',
            utils.y_true_header: 'y_true',
            utils.uncertainty_header: 'uncertainty'
        }
    ...
    threshold.plot_uncertainty(
        _df,
        kind='tile',
        threshold=tile_thresh,
        title=f'CV UQ Calibration: Exp {exp}'
    )
...
def thresholds_from_nested_cv(P, label, outer_k=3, inner_k=5, id=None,
    ...
    for k in range(1, outer_k+1):
        cv_dirs = utils.find_cv(P, f'{label}-k{k}', k=inner_k, outcome=outcome)
        k_preds = [join(f, 'tile_predictions_val_epoch1.csv') for f in cv_dirs]
        val_path = join(
            utils.find_model(P, f'{label}', kfold=k, outcome=outcome),
            'tile_predictions_val_epoch1.csv'
        )
        ...
        thresholds = threshold.from_cv(
            k_preds,
            tile_uq_thresh=tile_thresh,
            slide_uq_thresh='detect',
            **threshold_params
        )
        _, slide_thresh, tile_pred_thresh, slide_pred_thresh = thresholds
...
def add(path, label, out1, out2, outcome=utils.OUTCOME, order='forward',
    ...
    out1_indices = np.where((ann['site'].to_numpy() != 'GAN')
                            & (ann[outcome] == utils.OUTCOME1))[0]
    out2_indices = np.where((ann['site'].to_numpy() != 'GAN')
                            & (ann[outcome] == utils.OUTCOME2))[0]
...
def run(exp_to_run, steps=None, hp='nature2022'):
    ...
    for exp in exp_to_add:
        add(exp_annotations, label=exp, **exp_to_run[exp])
    ...
    if 1 in steps:
        ...
        for exp in full_epoch_exp:
            val_k = [
                ...
                if not utils.model_exists(P, f'EXP_{exp}', kfold=k)
            ]
            if not len(val_k):
    ...
    if 3 in steps:
        ...
        for exp in exp_to_run:
            val_k = [
                k for k in range(1, 4)
                if not utils.model_exists(P, f'EXP_{exp}_UQ', kfold=k)
            ]
            if not len(val_k):
...
def results(exp_to_run, uq=True, eval=True, plot=False):
    ...
    P = sf.Project(TRAIN_PATH)
    eval_Ps = [sf.Project(e) for e in EVAL_PATHS if e != 'not_configured']
    df = pd.DataFrame()
    ...
    for exp in exp_to_run:
        ...
        for i, m in enumerate(models):
            try:
                res = utils.get_model_results(m)
                pt_auc, pt_ap, slide_auc, slide_ap, tile_auc, tile_ap, _ = res
    ...
    for exp in exp_to_run:
        ...
        for i, m in enumerate(models):
            try:
                pt_auc, pt_ap, slide_auc, slide_ap, tile_auc, tile_ap, opt_thresh = utils.get_model_results(m)
    ...
    if uq:
        ...
        for exp in pb:
            ...
            try:
                _df, thresh = thresholds_from_nested_cv(
                    P, f'EXP_{exp}_UQ', id=exp
                )
            ...
            if plot and exp == 'AA':
                plot_uncertainty_calibration(
                    project=P,
                    exp=exp,
                    tile_thresh=thresh['tile_uq'],
                    slide_thresh=thresh['slide_uq'],
                    pred_thresh=thresh['slide_pred']
                )
        ...
        for val_P in eval_Ps:
            ...
            for exp in pb:
                ...
                try:
                    ...
                    pt_auc, pt_ap, slide_auc, slide_ap, tile_auc, tile_ap, _ = utils.get_model_results(eval_dir)
                ...
                patient_yt, patient_yp = utils.read_group_predictions(
                    join(
                        eval_dir,
                        f'patient_predictions_{utils.OUTCOME}_eval.csv'
                    )
                )
                ...
                model = utils.find_model(P, f'EXP_{exp}_FULL', epoch=1)
                ...
                if uq:
                    if exp in tile_uq_thresholds:
                        for keep in ('high_confidence', 'low_confidence'):
                            ...
                            new_cols = {
                                f'{utils.OUTCOME}_y_pred1': 'y_pred',
                                f'{utils.OUTCOME}_y_true0': 'y_true',
                                f'{utils.OUTCOME}_uncertainty1': 'uncertainty'
                            }
...
def display(df, eval_dfs, hue='uq', palette='tab10', relplot_uq_compare=True,
            boxplot_uq_compare=True, ttest_uq_groups=['all', 'include'], prefix=''):
